...
const PAGE_SIZE = 10; // 10 Serien x 2 Requests = 20 Subrequests

async function getTmdbId(title) {
  try {
    const res = await fetch(
      "https://api.themoviedb.org/3/search/tv?query=" + encodeURIComponent(title) + "&language=de-DE",
...
export async function onRequest(context) {
  ...
  await Promise.all(series.map(async (row) => {
    const newId = await getTmdbId(row.title);
    ...
    if (upd.ok) {
      updated.push({ title: row.title, old_id: row.tmdb_id, new_id: newId });
    } else {
      failed.push({ title: row.title, reason: "Update fehlgeschlagen" });
    }
  }));
